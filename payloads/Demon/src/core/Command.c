...
/* SMB */
        ...
        /* Check for new TCP pivot child connections and relay their output */
        TcpPivotPush();
...
VOID CommandPivot( PPARSER Parser )
{
    ...
    switch ( Pivot )
    {
        ...
        {
            DWORD Port    = ParserGetInt32( Parser );
            BOOL  Success = TcpPivotListen( Port );
        ...
        {
            DWORD AgentID = ParserGetInt32( Parser );
            BOOL  Success = TcpPivotRemove( AgentID );
        ...
        case DEMON_PIVOT_TCP_COMMAND:
        {
            ...
            if ( ! Data.Buffer || ! Data.Length ) {
                PUTS( "Can't send empty data to TCP pivot" )
                return;
            }

            if ( ! TcpPivotWrite( DemonId, Data.Buffer, Data.Length ) )
                PUTS( "TcpPivotWrite failed" )

            /* No response to teamserver for data relay */
            return;
        }
