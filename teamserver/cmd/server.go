...
var CobraServer = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if Server == nil {
			return fmt.Errorf("failed to create teamserver")
		}
		...
		logger.Info(fmt.Sprintf("Mugen Framework [Version: %v] [CodeName: %v]", VersionNumber, VersionName))

		if flags.Server.Default {
			Server.SetProfile(DirPath + "/data/mugen.yaotl")
