...
func (t *Teamserver) DispatchEvent(pk packager.Package) {
	...
	case packager.Type.Session.Type:
		...
		case packager.Type.Session.Input:
			...
			// track current task per agent for history persistence
			if taskID, ok := pk.Body.Info["TaskID"].(string); ok && taskID != "" {
				t.CurrentTaskID.Store(DemonID, taskID)
			}

			for i := range t.Agents.Agents {

				if t.Agents.Agents[i].DisplayID == DemonID {
					found = true
					...
					if t.Agents.Agents[i].Info.MagicValue == agent.DEMON_MAGIC_VALUE {
					...
					} else if t.Agents.Agents[i].Info.MagicValue == agent.TENGU_MAGIC_VALUE {
						...
						var Command string
						if cmd, ok := pk.Body.Info["Command"].(string); ok {
							Command = cmd
						} else if cmdline, ok := pk.Body.Info["CommandLine"].(string); ok {
							Command = cmdline
						}
						...
						if err := t.Agents.Agents[i].TenguTeamserverTaskPrepare(Command, taskID, Console); err != nil {
							Console(t.Agents.Agents[i].NameID, map[string]string{
								"Type":    "Error",
								"Message": "Failed to queue task: " + err.Error(),
							})
						}
						return
	...
	case packager.Type.Listener.Type:
		...
		case packager.Type.Listener.Add:
			...
			case handlers.AGENT_DNS:
				bindPort, _ := strconv.Atoi(fmt.Sprintf("%v", pk.Body.Info["BindPort"]))
				dnsConfig := handlers.DNSConfig{
					Name:     fmt.Sprintf("%v", pk.Body.Info["Name"]),
					BindHost: fmt.Sprintf("%v", pk.Body.Info["BindHost"]),
					BindPort: bindPort,
					Domain:   fmt.Sprintf("%v", pk.Body.Info["Domain"]),
				}
				if err := t.ListenerStart(handlers.LISTENER_DNS, dnsConfig); err != nil {
					t.Clients.Range(func(key, value any) bool {
						...
						client := value.(*Client)
						if client.Username == pk.Head.User {
							_ = t.SendEvent(id, events.Listener.ListenerError(pk.Head.User, dnsConfig.Name, err))
							return false
						}
						return true
					})
			...
			case handlers.AGENT_DOH:
				dohConfig := handlers.DoHConfig{
					Name:     fmt.Sprintf("%v", pk.Body.Info["Name"]),
					BindHost: fmt.Sprintf("%v", pk.Body.Info["BindHost"]),
					PortBind: fmt.Sprintf("%v", pk.Body.Info["PortBind"]),
					Domain:   fmt.Sprintf("%v", pk.Body.Info["Domain"]),
				}
				if fmt.Sprintf("%v", pk.Body.Info["Secure"]) == "true" {
					dohConfig.Secure = true
				}
				if err := t.ListenerStart(handlers.LISTENER_DOH, dohConfig); err != nil {
					t.Clients.Range(func(key, value any) bool {
						id := key.(string)
						client := value.(*Client)
						if client.Username == pk.Head.User {
							_ = t.SendEvent(id, events.Listener.ListenerError(pk.Head.User, dohConfig.Name, err))
							return false
						}
						return true
					})
			...
			case handlers.AGENT_PIVOT_TCP:
				pivotPort, _ := strconv.Atoi(fmt.Sprintf("%v", pk.Body.Info["PivotPort"]))
				tcpConfig := handlers.TCPConfig{
					Name:      fmt.Sprintf("%v", pk.Body.Info["Name"]),
					PivotHost: fmt.Sprintf("%v", pk.Body.Info["PivotHost"]),
					PivotPort: pivotPort,
				}
				if err := t.ListenerStart(handlers.LISTENER_PIVOT_TCP, tcpConfig); err != nil {
					t.Clients.Range(func(key, value any) bool {
						id := key.(string)
						client := value.(*Client)
						if client.Username == pk.Head.User {
							_ = t.SendEvent(id, events.Listener.ListenerError(pk.Head.User, tcpConfig.Name, err))
							return false
						}
						return true
					})
	...
	case packager.Type.Gate.Type:
		...
		case packager.Type.Gate.Stageless:
			...
			} else if AgentType == "Tengu" {
				go func() {
					var ConfigMap = make(map[string]any)
					err := json.Unmarshal([]byte(Config), &ConfigMap)
					if err != nil {
						logger.Error("Tengu: Failed to unmarshal config: " + err.Error())
						return
					}

					tenguBuilder := builder.NewTenguBuilder()
					tenguBuilder.ClientId = ClientID
					tenguBuilder.SendConsoleMessage = SendConsoleMsg
					tenguBuilder.SetConfig(ConfigMap)
					tenguBuilder.SetExtension(".elf")

					for i := 0; i < len(t.Listeners); i++ {
						if t.Listeners[i].Name == ListenerName {
							tenguBuilder.SetListener(t.Listeners[i].Type, t.Listeners[i].Config)
						}
					}

					if tenguBuilder.Build() {
						pal := tenguBuilder.GetPayloadBytes()
						if len(pal) > 0 {
							err := t.SendEvent(ClientID, events.Gate.SendStageless("tengu.elf", pal))
							if err != nil {
								logger.Error("Tengu: error sending payload event: " + err.Error())
								return
							}
							tenguBuilder.DeletePayload()
						}
					}
				}()
			} else {
	...
	case packager.Type.TaskHistory.Type:

		switch pk.Body.SubEvent {

		case packager.Type.TaskHistory.List:
		...
		case packager.Type.TaskHistory.SetComment:
			...
			comment, ok2 := pk.Body.Info["Comment"].(string)
			if !ok1 || taskID == "" || !ok2 {
				return
			}
			t.DB.TaskSetComment(taskID, comment)

		case packager.Type.TaskHistory.Delete:
			taskID, ok := pk.Body.Info["TaskID"].(string)
			if !ok || taskID == "" {
				return
			}
