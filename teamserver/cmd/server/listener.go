...
func (t *Teamserver) ListenerStart(ListenerType int, info any) error {
	...
	for _, listener := range t.Listeners {
		...
		case handlers.LISTENER_DOH:
			Name = info.(handlers.DoHConfig).Name
		case handlers.LISTENER_PIVOT_TCP:
			Name = info.(handlers.TCPConfig).Name
		}
	...
	case handlers.LISTENER_DNS:
		dnsHandler := handlers.NewDNS()
		dnsHandler.Config = info.(handlers.DNSConfig)
		dnsHandler.Teamserver = t
		dnsHandler.Start()

		ListenerConfig = dnsHandler
		ListenerName = dnsHandler.Config.Name

		break

	case handlers.LISTENER_DOH:
		dohHandler := handlers.NewDoH()
		dohHandler.Config = info.(handlers.DoHConfig)
		dohHandler.Teamserver = t
		dohHandler.Start()

		ListenerConfig = dohHandler
		ListenerName = dohHandler.Config.Name

		break

	case handlers.LISTENER_PIVOT_TCP:
		tcp := &handlers.TCP{}
		tcp.Config = info.(handlers.TCPConfig)
		tcp.Teamserver = t

		tcp.Start()

		ListenerConfig = tcp
		ListenerName = tcp.Config.Name
...
func (t *Teamserver) ListenerRemove(Name string) ([]*Listener, []packager.Package) {
	for i := range t.Listeners {
		if t.Listeners[i].Name == Name {
			...
			case *handlers.DNS:
				t.Listeners[i].Config.(*handlers.DNS).Stop()
			case *handlers.DoH:
				t.Listeners[i].Config.(*handlers.DoH).Stop()
			}
...
func (t *Teamserver) ListenerAdd(FromUser string, Type int, Config any) packager.Package {
	...
	case handlers.LISTENER_PIVOT_TCP:
		...
		Info := map[string]interface{}{
			"PivotHost": Config.(*handlers.TCP).Config.PivotHost,
			"PivotPort": fmt.Sprintf("%d", Config.(*handlers.TCP).Config.PivotPort),
			"Status":    "Online",
		}

		Protocol = handlers.AGENT_PIVOT_TCP
		Name = Config.(*handlers.TCP).Config.Name
