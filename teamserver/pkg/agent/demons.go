...
func (a *Agent) TaskPrepare(Command int, Info any, Message *map[string]string, ClientID string, teamserver TeamServer) (*Job, error) {
	...
	case COMMAND_PIVOT:
		...
		case DEMON_PIVOT_TCP_LISTEN:
			var Port int
			if val, err2 := strconv.Atoi(Param); err2 == nil {
				Port = val
			}
			job.Data = []interface{}{
				PivotCommand,
		...
		case DEMON_PIVOT_TCP_DISCONNECT:
			var AgentIDTcp, err2 = strconv.ParseInt(Param, 16, 32)
			if err2 != nil {
				return nil, err2
			}
			job.Data = []interface{}{
			...
			case DEMON_COMMAND_PROC_MODULES:
				...
				if Parser.CanIRead([]parser.ReadType{parser.ReadInt32}) {
					...
					for Parser.CanIRead([]parser.ReadType{parser.ReadBytes, parser.ReadPointer}) {
						...
						collum = []string{strings.ReplaceAll(ModuleName, " ", ""), ModuleBase} // TODO: fix this to avoid new line in the mugen console
						tableData = append(tableData, collum)
					}
	...
	case COMMAND_INLINEEXECUTE:
		...
		case CALLBACK_OUTPUT:
			if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
				...
				teamserver.AgentConsole(a.DisplayID, MUGEN_CONSOLE_MESSAGE, OutputMap)
			} else {
		...
		case COMMAND_INLINEEXECUTE_RAN_OK:
			...
			for i, BofCallback := range a.BofCallbacks {
				if BofCallback.TaskID == RequestID {
					...
					teamserver.PythonModuleCallback(BofCallback.ClientID, a.DisplayID, MUGEN_BOF_CALLBACK, OutputMap)
		...
		if Parser.CanIRead([]parser.ReadType{parser.ReadInt32}) {
			...
			case DEMON_PIVOT_TCP_LISTEN:
				if Parser.CanIRead([]parser.ReadType{parser.ReadInt32}) {
					...
					if Success == win32.TRUE {
						Message["Type"] = "Good"
						Message["Message"] = "[TCP] Pivot listener started"
					} else {
						Message["Type"] = "Error"
						Message["Message"] = "[TCP] Failed to start pivot listener"
					}
					a.RequestCompleted(RequestID)
				}
				break

			case DEMON_PIVOT_TCP_CONNECT:
				if Parser.CanIRead([]parser.ReadType{parser.ReadInt32}) {
					var Success = Parser.ParseInt32()
					...
					if Success == 1 {
						if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
							var (
								AgentData = Parser.ParseBytes()
								AgentHdr  Header
								err       error
							)

							if AgentHdr, err = ParseHeader(AgentData); err == nil {

								var PivotAgent *Agent

								if AgentHdr.MagicValue == DEMON_MAGIC_VALUE {
									/* Demon child connected via TCP */
									if teamserver.AgentExist(AgentHdr.AgentID) {
										PivotAgent = teamserver.AgentInstance(AgentHdr.AgentID)
										Message["MiscType"] = "reconnect"
										Message["MiscData"] = fmt.Sprintf("%v;%x", a.NameID, AgentHdr.AgentID)
										...
										if PivotAgent.Pivots.Parent != nil {
											for i := range PivotAgent.Pivots.Parent.Pivots.Links {
												if PivotAgent.Pivots.Parent.Pivots.Links[i].NameID == fmt.Sprintf("%08x", AgentHdr.AgentID) {
													PivotAgent.Pivots.Parent.Pivots.Links = append(PivotAgent.Pivots.Parent.Pivots.Links[:i], PivotAgent.Pivots.Parent.Pivots.Links[i+1:]...)
													break
												}
											}
										}

										PivotAgent.Active = true
										PivotAgent.Reason = ""
										PivotAgent.Pivots.Parent = a

										a.Pivots.Links = append(a.Pivots.Links, PivotAgent)
										teamserver.LinkAdd(a, PivotAgent)
										teamserver.AgentUpdate(PivotAgent)
										teamserver.AgentUpdate(a)
										...
										PivotAgent = ParseDemonRegisterRequest(AgentHdr.AgentID, AgentHdr.Data, "")
										PivotAgent.Pivots.Parent = a
										PivotAgent.Info.MagicValue = AgentHdr.MagicValue

										a.Pivots.Links = append(a.Pivots.Links, PivotAgent)
										teamserver.LinkAdd(a, PivotAgent)
										teamserver.AgentAdd(PivotAgent)
										teamserver.AgentSendNotify(PivotAgent)
									}

									Message["Type"] = "Good"
									Message["Message"] = "[TCP] Connected to Demon pivot agent [" + a.NameID + "]-<>-<>-[" + PivotAgent.NameID + "]"

								} else if AgentHdr.MagicValue == TENGU_MAGIC_VALUE {
									/* Tengu child connected via TCP */
									if teamserver.AgentExist(AgentHdr.AgentID) {
										PivotAgent = teamserver.AgentInstance(AgentHdr.AgentID)
										Message["MiscType"] = "reconnect"
										Message["MiscData"] = fmt.Sprintf("%v;%x", a.NameID, AgentHdr.AgentID)
										...
										if PivotAgent == nil {
											Message["Type"] = "Error"
											Message["Message"] = "[TCP] Failed to parse Tengu registration"
											break
										}
										PivotAgent.Pivots.Parent = a
									...
									/* Queue the init response (confirmed agent ID) for parent to relay */
									if PivotAgent != nil {
										childID, _ := strconv.ParseInt(PivotAgent.NameID, 16, 64)
										initResp := make([]byte, 4)
										initResp[0] = byte(childID)
										initResp[1] = byte(childID >> 8)
										initResp[2] = byte(childID >> 16)
										initResp[3] = byte(childID >> 24)

										a.JobQueue = append(a.JobQueue, Job{
											Command: COMMAND_PIVOT,
											Data: []interface{}{
												DEMON_PIVOT_TCP_COMMAND,
												uint32(childID),
												initResp,
											},
										})
										...
										Message["Type"] = "Good"
										Message["Message"] = "[TCP] Connected to Tengu pivot agent [" + a.NameID + "]-<>-<>-[" + PivotAgent.NameID + "]"
									}

								} else {
									Message["Type"] = "Error"
									Message["Message"] = "[TCP] Unknown magic value"
								}

							} else {
								Message["Type"] = "Error"
								Message["Message"] = "[TCP] Failed to parse agent header: " + err.Error()
							}
						}
					} else {
						Message["Type"] = "Error"
						Message["Message"] = "[TCP] Child failed to connect"
					}
				}
				break
			...
			case DEMON_PIVOT_TCP_DISCONNECT:
				if Parser.CanIRead([]parser.ReadType{parser.ReadInt32, parser.ReadInt32}) {
					...
					if Success == win32.TRUE {
						Message["Type"] = "Info"
						Message["Message"] = fmt.Sprintf("[TCP] Pivot agent disconnected %x", TcpAgentID)
						Message["MiscType"] = "disconnect"
						Message["MiscData"] = fmt.Sprintf("%08x", TcpAgentID)

						AgentInstance := teamserver.AgentInstance(TcpAgentID)
						if AgentInstance != nil {
							teamserver.LinkRemove(a, AgentInstance, true)
						}
					} else {
						Message["Type"] = "Error"
						Message["Message"] = fmt.Sprintf("[TCP] Failed to disconnect pivot agent %x", TcpAgentID)
					}
					a.RequestCompleted(RequestID)
				}
				break

			case DEMON_PIVOT_TCP_COMMAND:
				if Parser.CanIRead([]parser.ReadType{parser.ReadBytes}) {
					var (
						Package       = Parser.ParseBytes()
						AgentHdr, err = ParseHeader(Package)
					...
					if err == nil {
						if AgentHdr.MagicValue == DEMON_MAGIC_VALUE {
							/* Relay for Demon child - same as SMB */
							var PivotAgent = teamserver.AgentInstance(AgentHdr.AgentID)
							if PivotAgent != nil {
								PivotAgent.UpdateLastCallback(teamserver)
								...
								Message["Type"] = "Error"
								Message["Message"] = fmt.Sprintf("[TCP] Demon pivot agent %x not found", AgentHdr.AgentID)
							}

						} else if AgentHdr.MagicValue == TENGU_MAGIC_VALUE {
							/* Relay for Tengu child */
							responseBytes := TenguHandlePivotFrame(teamserver, AgentHdr)
							if len(responseBytes) > 0 {
								childID, _ := strconv.ParseInt(fmt.Sprintf("%08x", AgentHdr.AgentID), 16, 64)
								a.JobQueue = append(a.JobQueue, Job{
									Command: COMMAND_PIVOT,
									Data: []interface{}{
										DEMON_PIVOT_TCP_COMMAND,
							...
							Message["Type"] = "Error"
							Message["Message"] = "[TCP] Unknown agent magic value"
						}

					} else {
						Message["Type"] = "Error"
						Message["Message"] = "[TCP] Failed to parse pivot frame: " + err.Error()
					}
				}
				break
...
func (a *Agent) Console(Console func(DemonID string, CommandID int, Output map[string]string), Type, Text, Output string) {
	...
	Console(a.NameID, MUGEN_CONSOLE_MESSAGE, Message)
}
