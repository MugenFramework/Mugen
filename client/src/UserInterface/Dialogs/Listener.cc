...
using namespace MugenNamespace::UserInterface::Dialogs;
...
NewListener::NewListener( QDialog* Dialog )
{
    ...
    ComboHostBind->addItems( QStringList() << MugenX::Teamserver.IpAddresses << "127.0.0.1" << "0.0.0.0" );
    ...
    auto style = QString( "color: #2a2a3f;" );
    ...
    LabelDNSBindHost = new QLabel( PageDNS );
    ComboDNSBindHost = new QComboBox( PageDNS );
    LabelDNSBindPort = new QLabel( PageDNS );
    InputDNSBindPort = new QLineEdit( PageDNS );
    LabelDNSDomain   = new QLabel( PageDNS );
    InputDNSDomain   = new QLineEdit( PageDNS );

    ComboDNSBindHost->addItems( QStringList() << MugenX::Teamserver.IpAddresses << "0.0.0.0" );
    InputDNSBindPort->setText( "53" );
    InputDNSBindPort->setPlaceholderText( "53" );
    InputDNSDomain->setPlaceholderText( "c2.example.com" );

    LabelDNSBindHost->setText( "Bind Host:" );
    LabelDNSBindPort->setText( "Bind Port:" );
    LabelDNSDomain->setText( "Domain:" );

    formLayout_DNS->setWidget( 0, QFormLayout::LabelRole, LabelDNSBindHost );
    formLayout_DNS->setWidget( 0, QFormLayout::FieldRole, ComboDNSBindHost );
    formLayout_DNS->setWidget( 1, QFormLayout::LabelRole, LabelDNSBindPort );
    formLayout_DNS->setWidget( 1, QFormLayout::FieldRole, InputDNSBindPort );
    formLayout_DNS->setWidget( 2, QFormLayout::LabelRole, LabelDNSDomain );
    formLayout_DNS->setWidget( 2, QFormLayout::FieldRole, InputDNSDomain );

    // =============
    // ==== DoH ====
    // =============
    PageDoH        = new QWidget();
    formLayout_DoH = new QFormLayout( PageDoH );
    LabelDoHBindHost = new QLabel( PageDoH );
    ComboDoHBindHost = new QComboBox( PageDoH );
    LabelDoHPortBind = new QLabel( PageDoH );
    InputDoHPortBind = new QLineEdit( PageDoH );
    LabelDoHDomain   = new QLabel( PageDoH );
    InputDoHDomain   = new QLineEdit( PageDoH );
    LabelDoHSecure   = new QLabel( PageDoH );
    CheckDoHSecure   = new QCheckBox( PageDoH );

    ComboDoHBindHost->addItems( QStringList() << MugenX::Teamserver.IpAddresses << "0.0.0.0" );
    InputDoHPortBind->setText( "443" );
    InputDoHPortBind->setPlaceholderText( "443" );
    InputDoHDomain->setPlaceholderText( "c2.example.com" );
    CheckDoHSecure->setChecked( true );

    LabelDoHBindHost->setText( "Bind Host:" );
    LabelDoHPortBind->setText( "Bind Port:" );
    LabelDoHDomain->setText( "Domain:" );
    LabelDoHSecure->setText( "HTTPS:" );
    CheckDoHSecure->setText( "Enable TLS" );

    formLayout_DoH->setWidget( 0, QFormLayout::LabelRole, LabelDoHBindHost );
    formLayout_DoH->setWidget( 0, QFormLayout::FieldRole, ComboDoHBindHost );
    formLayout_DoH->setWidget( 1, QFormLayout::LabelRole, LabelDoHPortBind );
    formLayout_DoH->setWidget( 1, QFormLayout::FieldRole, InputDoHPortBind );
    formLayout_DoH->setWidget( 2, QFormLayout::LabelRole, LabelDoHDomain );
    formLayout_DoH->setWidget( 2, QFormLayout::FieldRole, InputDoHDomain );
    formLayout_DoH->setWidget( 3, QFormLayout::LabelRole, LabelDoHSecure );
    formLayout_DoH->setWidget( 3, QFormLayout::FieldRole, CheckDoHSecure );
    ...
    // =================
    PageTcp        = new QWidget();
    formLayout_Tcp = new QFormLayout( PageTcp );
    LabelTcpHost   = new QLabel( PageTcp );
    InputTcpHost   = new QLineEdit( PageTcp );
    LabelTcpPort   = new QLabel( PageTcp );
    InputTcpPort   = new QLineEdit( PageTcp );

    InputTcpHost->setPlaceholderText( "192.168.1.50" );
    InputTcpPort->setText( "4444" );
    InputTcpPort->setPlaceholderText( "4444" );

    LabelTcpHost->setText( "Parent Host:" );
    LabelTcpPort->setText( "Parent Port:" );

    formLayout_Tcp->setWidget( 0, QFormLayout::LabelRole, LabelTcpHost );
    formLayout_Tcp->setWidget( 0, QFormLayout::FieldRole, InputTcpHost );
    formLayout_Tcp->setWidget( 1, QFormLayout::LabelRole, LabelTcpPort );
    formLayout_Tcp->setWidget( 1, QFormLayout::FieldRole, InputTcpPort );
    ...
    StackWidgetConfigPages->addWidget( PageDoH );
    StackWidgetConfigPages->addWidget( PageTcp );
    ...
    ComboPayload->addItem( "DoH" );
    ComboPayload->addItem( "Tcp" );
    ...
    QObject::connect( ButtonHostsGroupAdd, &QPushButton::clicked, this, [&]()
    {
        ...
        if ( HostsData.size() == 0 ) {
            if ( ! MugenX::Teamserver.IpAddresses.isEmpty() ) {
                Item->setText( MugenX::Teamserver.IpAddresses[ 0 ] );
            }
        }
    ...
    {
        if ( text.compare( MugenSpace::Listener::PayloadHTTPS ) == 0 )
        {
            StackWidgetConfigPages->setCurrentIndex( 0 );
            InputPortBind->setText( "443" );
            InputPortConn->setText( "443" );
        }
        else if ( text.compare( MugenSpace::Listener::PayloadHTTP ) == 0 )
        {
            StackWidgetConfigPages->setCurrentIndex( 0 );
            InputPortBind->setText( "80" );
            InputPortConn->setText( "80" );
        }
        else if ( text.compare( MugenSpace::Listener::PayloadSMB ) == 0 )
        {
            StackWidgetConfigPages->setCurrentIndex( 1 );
        }
        else if ( text.compare( MugenSpace::Listener::PayloadExternal ) == 0 )
        {
            StackWidgetConfigPages->setCurrentIndex( 2 );
        }
        else if ( text.compare( MugenSpace::Listener::PayloadDNS ) == 0 )
        {
            StackWidgetConfigPages->setCurrentIndex( 3 );
        }
        else if ( text.compare( MugenSpace::Listener::PayloadDoH ) == 0 )
        {
            StackWidgetConfigPages->setCurrentIndex( 4 );
        }
        else if ( text.compare( MugenSpace::Listener::PayloadTcp ) == 0 )
        {
            StackWidgetConfigPages->setCurrentIndex( 5 );
        }
        else
        {
...
MapStrStr NewListener::Start( Util::ListenerItem Item, bool Edit )
{
    ...
    if ( ( Payload.compare( MugenSpace::Listener::PayloadHTTPS ) == 0 ) || ( Payload.compare( MugenSpace::Listener::PayloadHTTP ) == 0 ) )
    {
        auto Hosts   = std::string();
        auto Headers = std::string();
        auto Uris    = std::string();

        if ( Payload.compare( MugenSpace::Listener::PayloadHTTPS ) == 0 )
    ...
    else if ( Payload.compare( MugenSpace::Listener::PayloadSMB ) == 0 )
    {
        ListenerInfo.insert( { "PipeName", InputPipeName->text().toStdString() } );
    }
    else if ( Payload.compare( MugenSpace::Listener::PayloadExternal ) == 0 )
    {
        for ( auto& Listener : MugenX::Teamserver.Listeners )
        {
            if ( Listener.Protocol == MugenSpace::Listener::PayloadExternal.toStdString() )
            {
                if ( any_cast<MugenSpace::Listener::External>( Listener.Info ).Endpoint.compare( InputEndpoint->text() ) == 0 )
                {
    ...
    else if ( Payload.compare( MugenSpace::Listener::PayloadDNS ) == 0 )
    {
        ListenerInfo.insert( { "BindHost", ComboDNSBindHost->currentText().toStdString() } );
        ListenerInfo.insert( { "BindPort", InputDNSBindPort->text().toStdString() } );
        ListenerInfo.insert( { "Domain",   InputDNSDomain->text().toStdString() } );
    }
    else if ( Payload.compare( MugenSpace::Listener::PayloadDoH ) == 0 )
    {
        ListenerInfo.insert( { "BindHost", ComboDoHBindHost->currentText().toStdString() } );
        ListenerInfo.insert( { "PortBind", InputDoHPortBind->text().toStdString() } );
        ListenerInfo.insert( { "Domain",   InputDoHDomain->text().toStdString() } );
        ListenerInfo.insert( { "Secure",   CheckDoHSecure->isChecked() ? "true" : "false" } );
    }
    else if ( Payload.compare( MugenSpace::Listener::PayloadTcp ) == 0 )
    {
        ListenerInfo.insert( { "PivotHost", InputTcpHost->text().toStdString() } );
        ListenerInfo.insert( { "PivotPort", InputTcpPort->text().toStdString() } );
    }
    else
    {
        for ( const auto& listener : ServiceListeners )
        {
            if ( listener.Name == Payload.toStdString() )
            {
                auto Listener = MapStrStr{
                    ...
                    { "ClientUser", MugenX::Teamserver.User.toStdString() },
                };
...
auto NewListener::ListenerCustomAdd( QString Json ) -> bool
{
    ...
    for ( auto& x : MugenX::Teamserver.RegisteredListeners )
    {
...
void MugenNamespace::UserInterface::Dialogs::NewListener::onButton_Save()
{
    auto Payload = ComboPayload->currentText();

    if ( ( Payload.compare( MugenSpace::Listener::PayloadHTTPS ) == 0 ) ||
         ( Payload.compare( MugenSpace::Listener::PayloadHTTP  ) == 0 ) )
    {
    ...
    {
        if ( InputDNSBindPort->text().isEmpty() || ! is_number( InputDNSBindPort->text().toStdString() ) )
        {
            MessageBox( "Listener Error", "Invalid DNS Bind Port", QMessageBox::Critical );
            return;
        }
        if ( InputDNSDomain->text().isEmpty() )
        {
            MessageBox( "Listener Error", "No Domain specified", QMessageBox::Critical );
            return;
        }
    }
    else if ( Payload.compare( MugenSpace::Listener::PayloadDoH ) == 0 )
    {
        if ( InputDoHPortBind->text().isEmpty() || ! is_number( InputDoHPortBind->text().toStdString() ) )
        {
            MessageBox( "Listener Error", "Invalid DoH Bind Port", QMessageBox::Critical );
            return;
        }
        if ( InputDoHDomain->text().isEmpty() )
        {
            MessageBox( "Listener Error", "No Domain specified", QMessageBox::Critical );
            return;
        }
    ...
    {
        if ( InputTcpHost->text().isEmpty() )
        {
            MessageBox( "Listener Error", "No Parent Host specified", QMessageBox::Critical );
            return;
        }
        if ( InputTcpPort->text().isEmpty() || ! is_number( InputTcpPort->text().toStdString() ) )
        {
            MessageBox( "Listener Error", "Invalid Parent Port", QMessageBox::Critical );
            return;
        }
    }
    else
...
void MugenNamespace::UserInterface::Dialogs::NewListener::onProxyEnabled()
{
    if ( CheckEnableProxy->isChecked() )
    {
        ProxyConfigBox->setEnabled( true );

        auto style = QString( "color: #f0f0ee;" );
        LabelProxyType->setStyleSheet( style );
        LabelProxyHost->setStyleSheet( style );
        LabelProxyPort->setStyleSheet( style );
        LabelUserName->setStyleSheet( style );
