...
auto Payload::retranslateUi() -> void
{
    ...
    if ( MugenX::Teamserver.Listeners.size() > 0 )
    {
    ...
    ComboAgentType->addItem( "Tengu" );

    for ( auto& Agents : MugenX::Teamserver.ServiceAgents )
...
auto Payload::CtxAgentPayloadChange( const QString& AgentType ) -> void
{
    if ( ! Closed )
    {
        ...
        else if ( AgentType.compare( "Tengu" ) == 0 )
        {
            TenguDefaultConfig();

            ComboFormat->clear();
            ComboFormat->addItem( "Linux ELF" );
...
auto Payload::DefaultConfig() -> void
{
    ...
    auto DemonConfig             = MugenX::Teamserver.DemonConfig;
...
auto Payload::TenguDefaultConfig() -> void
{
    TreeConfig->clear();
