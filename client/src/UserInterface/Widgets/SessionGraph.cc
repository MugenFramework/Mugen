    ...
    for ( QGraphicsItem* g_item : items )
    {
        if ( qgraphicsitem_cast<Edge*>( g_item ) )
        {
            ...
            if ( i->dest->NodeID.compare( AgentID ) == 0 )
            {
                ...
                i->Color( QColor( MugenNamespace::Util::ColorText::Colors::Hex::Red ) );
...
void GraphWidget::GraphPivotNodeReconnect( QString ParentAgentID, QString ChildAgentID )
{
    ...
    for ( QGraphicsItem* g_item : items )
    {
        if ( qgraphicsitem_cast<Edge*>( g_item ) )
        {
            ...
            {
                GraphScene->addItem( new Edge( GraphNodeGet( ParentAgentID ), i->dest, QColor( MugenNamespace::Util::ColorText::Colors::Hex::Purple ) ) );
                GraphScene->removeItem( i );

                // TODO: somehow remove/free i (Edge*)
                // i->source = GraphNodeGet( ParentAgentID );
                // i->dest->Disconnected = false;
                // i->Color( QColor( MugenNamespace::Util::ColorText::Colors::Hex::Purple ) );
...
void GraphWidget::drawBackground( QPainter* painter, const QRectF& rect )
{
    ...
    auto Background = MugenNamespace::Util::ColorText::Colors::Hex::Background;
            ...
            if ( Session.Name.compare( NodeID ) == 0 )
            {
                ...
                else if ( action->text().compare( "Remove" ) == 0 )
                {
                    ...
                    for ( int i = 0; i < MugenX::Teamserver.TabSession->SessionTableWidget->SessionTableWidget->rowCount(); i++ )
                    {
                        ...
                        {
                            MugenX::Teamserver.TabSession->SessionTableWidget->SessionTableWidget->removeRow( i );
                        }
                    }
...
void Node::paint( QPainter *painter, const QStyleOptionGraphicsItem* option, QWidget* )
{
    ...
    {
        auto image1 = QImage( ":/images/SessionMugen" );
