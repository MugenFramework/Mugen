...
auto PathGetParent( QString MainPath ) -> QString
{
    ...
    bool isLinux = ( Path.find('/') != std::string::npos );
    char sep     = isLinux ? '/' : '\\';

    auto Indx = Path.rfind( sep );
    if ( Indx == std::string::npos )
        return QString( Path.c_str() );

    // Linux root edge case: keep the leading /
    if ( isLinux && Indx == 0 )
        return "/";
...
void FileBrowser::onTableDoubleClick( int row, int column )
{
    ...
    {
        QString sep  = IsTengu ? "/" : "\\";
        QString Path = Item->Data.Path + sep + Item->Data.Name;
        TableClear();
        ...
        if ( Session.Name.compare( SessionID ) == 0 )
        {
            if ( IsTengu )
            {
                ...
                if ( Session.InteractedWidget->TenguCmds )
                    Session.InteractedWidget->TenguCmds->SendTask( TaskID, "ls;ui " + Path );
            }
            else
            {
        ...
        for ( auto& Session : MugenX::Teamserver.Sessions )
        {
            if ( Session.Name.compare( SessionID ) == 0 )
            {
                if ( IsTengu )
                {
                    auto TaskID = QString( Util::gen_random( 8 ).c_str() );
                    if ( Session.InteractedWidget->TenguCmds )
                        Session.InteractedWidget->TenguCmds->SendTask( TaskID, "download " + Path );
                }
                else
                {
