...
{
    StoreWidget = Store;
    QUrl url("https://raw.githubusercontent.com/MugenFramework/Mugen/main/public/mugen-modules.json");
    ...
    headerLabelTitle = new QLabel( "<h1>Mugen Extensions!</h1>", panelStore );
    ...
    panelLabelDescription = new QLabel( "This tab is to install extensions inside of Mugen!", panelStore );
...
void Store::installScript(int position)
{
    ...
    if (is_gist != -1) {
        ...
        if ( AddScript( pathScript ) ) {
            if ( ! MugenX::Teamserver.TabSession->dbManager->CheckScript(pathScript) )
                MugenX::Teamserver.TabSession->dbManager->AddScript(pathScript);
        }
