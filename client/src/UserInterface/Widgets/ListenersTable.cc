...
void MugenNamespace::UserInterface::Widgets::ListenersTable::setupUi( QWidget* Form )
{
...
void MugenNamespace::UserInterface::Widgets::ListenersTable::ButtonsInit()
{
    QObject::connect( buttonAdd, &QPushButton::clicked, this, [&]()
    {
        auto ListenerDialog = new UserInterface::Dialogs::NewListener( new QDialog( this->ListenerWidget->window() ) );
        auto ListenerInfo   = MapStrStr();

        /* add custom listeners to it. */
        for ( const auto& listenerService : MugenX::Teamserver.RegisteredListeners )
    ...
    QObject::connect( buttonEdit, &QPushButton::clicked, this, [&]()
    {
        ...
        ListenerDialog = new UserInterface::Dialogs::NewListener( new QDialog( this->ListenerWidget->window() ) );
    ...
    QObject::connect( buttonRemove,  &QPushButton::clicked, this, [&]()
    {
        ...
        for ( int i = 0; i < MugenX::Teamserver.Listeners.size(); i++ )
        {
            if ( MugenX::Teamserver.Listeners[ i ].Name == Name )
            {
                MugenX::Teamserver.Listeners.erase( MugenX::Teamserver.Listeners.begin() + i );
            }
        }
...
void MugenNamespace::UserInterface::Widgets::ListenersTable::ListenerAdd( Util::ListenerItem item ) const
{
    for ( auto& listener : MugenX::Teamserver.Listeners )
    {
    ...
    {
        item_Host->setText( any_cast<Listener::DNS>( item.Info ).Domain );
        item_PortBind->setText( any_cast<Listener::DNS>( item.Info ).BindPort );
    }
    else if ( item.Protocol == Listener::PayloadDoH.toStdString() )
    {
        item_Host->setText( any_cast<Listener::DoH>( item.Info ).Domain );
        item_PortBind->setText( any_cast<Listener::DoH>( item.Info ).PortBind );
    }
    else if ( item.Protocol == Listener::PayloadTcp.toStdString() )
    {
        item_Host->setText( any_cast<Listener::Tcp>( item.Info ).PivotHost );
        item_PortBind->setText( any_cast<Listener::Tcp>( item.Info ).PivotPort );
    }
    else
    {
        auto Host     = QString();
    ...
    MugenX::Teamserver.Listeners.push_back( item );
}

void MugenNamespace::UserInterface::Widgets::ListenersTable::setDBManager( MugenSpace::DBManager* dbManager )
{
...
void UserInterface::Widgets::ListenersTable::ListenerEdit( Util::ListenerItem item ) const
{
    for ( int i = 0; i < MugenX::Teamserver.Listeners.size(); i++ )
    {
        if ( MugenX::Teamserver.Listeners[ i ].Name == item.Name )
        {
            MugenX::Teamserver.Listeners[ i ].Info = item.Info;
        }
    }
}
...
void UserInterface::Widgets::ListenersTable::ListenerRemove( QString ListenerName ) const
{
    ...
    if ( ! ListenerName.isEmpty() )
    {
        ...
        if ( ! MugenX::Teamserver.Listeners.empty() )
        {
            for ( int i = 0; i < MugenX::Teamserver.Listeners.size(); i++ )
            {
                if ( MugenX::Teamserver.Listeners[ i ].Name == ListenerName.toStdString() )
                {
