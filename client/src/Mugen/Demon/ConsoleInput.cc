    ...
    if ( IsDemonAgent )
    {
        if ( InputCommands[ 0 ].compare( "help" ) == 0 )
        {
            if ( InputCommands.size() > 1 && InputCommands[ 1 ] != "" )
            {
                ...
                for ( auto & commandIndex : DemonCommandList )
                {
                    if ( InputCommands[ 1 ].compare( commandIndex.CommandString ) == 0 )
                    {
                        ...
                        if ( ( ! commandIndex.SubCommands.empty() || commandIndex.Module ) && InputCommands.size() > 2 && InputCommands[ 2 ] != "" )
                        {
                            ...
                            if ( ! FoundSubCommand )
                            {
                                for ( auto& Command : MugenX::Teamserver.RegisteredCommands )
                                {
                ...
                if ( ! FoundCommand )
                {
                    ...
                    for ( auto& Module : MugenX::Teamserver.RegisteredModules )
                    {
        ...
        else if ( InputCommands[ 0 ].compare( "pivot" ) == 0 )
        {
            if ( InputCommands.size() > 1 )
            {
                ...
                else if ( InputCommands[ 1 ].compare( "tcp" ) == 0 )
                {
                    if ( InputCommands.size() < 3 )
                    {
                        CONSOLE_ERROR( "Usage: pivot tcp [listen <port> | disconnect <agent_id>]" )
                        return false;
                    }

                    if ( InputCommands[ 2 ].compare( "listen" ) == 0 )
                    {
                        if ( InputCommands.size() < 4 )
                        {
                            CONSOLE_ERROR( "Usage: pivot tcp listen <port>" )
                            return false;
                        }

                        Command = "20";
                        Param   = InputCommands[ 3 ];
                        TaskID  = CONSOLE_INFO( "Tasked demon to listen for TCP pivot on port " + Param );
                    }
                    else if ( InputCommands[ 2 ].compare( "disconnect" ) == 0 )
                    {
                        if ( InputCommands.size() < 4 )
                        {
                            CONSOLE_ERROR( "Usage: pivot tcp disconnect <agent_id>" )
                            return false;
                        }

                        Command = "22";
                        Param   = InputCommands[ 3 ];
                        TaskID  = CONSOLE_INFO( "Tasked demon to disconnect TCP pivot agent: " + Param );
                    }
                    else
                    {
                        CONSOLE_ERROR( "Unknown tcp pivot sub-command: " + InputCommands[ 2 ] )
                        return false;
                    }
                }
                else
        ...
        else if ( InputCommands[ 0 ].compare( "havoc" ) == 0 )
        {
            ...
            art( ColorText::Colors::Hex::Purple, "  #   # #   #   #    ###   ###  " );
            DemonConsole->Console->append( "" );
            DemonConsole->Console->append( Util::ColorText::Comment( "  Mugen is built on the shoulders of giants." ) );
            DemonConsole->Console->append( Util::ColorText::Comment( "  Havoc (GPL-3.0, December 2025) laid the foundation." ) );
            DemonConsole->Console->append( Util::ColorText::Comment( "  No paywalls, no closed doors. The spirit lives on." ) );
            DemonConsole->Console->append( "" );
            return true;
        }
        else if ( InputCommands[ 0 ].compare( "5pider" ) == 0 || InputCommands[ 0 ].compare( "c5pider" ) == 0 )
        {
            auto art = [&]( const QString& hex, const QString& line ) {
                DemonConsole->Console->append(
                    "<span style=\"color:" + hex + ";\">" +
            ...
            art( ColorText::Colors::Hex::Pink, "  ###  #    ###  ###  ###  # #" );
            DemonConsole->Console->append( "" );
            DemonConsole->Console->append( Util::ColorText::Comment( "  @C5pider - architect of Havoc, father of Demon." ) );
            DemonConsole->Console->append( Util::ColorText::Comment( "  His work lives on in every shellcode we execute." ) );
            DemonConsole->Console->append( Util::ColorText::Comment( "  Mugen would not exist without him." ) );
            DemonConsole->Console->append( "" );
            return true;
        }
