...
- Console command output: agent stdout is rendered as a guttered block (`│ `), HTML-escaped, and wrapped to the console width. Spaces are kept (`&nbsp;` + monospace) so `ls` / `ps` tables stay aligned. Status lines (`[*]` / `[+]` / `[!]`) stay as they were. Tengu `ls` and `ps` print aligned tables (permissions/size/mtime, or PID/user/image) instead of a raw name dump. `ls;ui` / `ps;ui` still open the File Explorer and Process List.
- Console `ls` has a **Tasks** column: directories show a clickable `ls` that sends `ls <that folder>` in the same console. Files have an empty Tasks cell.
...
- Ops hub: **View → Ops** is a single bottom tab with an internal navbar (Callbacks, Screenshots, Credentials, Downloads, Resources, Tasks, Networking). The old separate View entries for Loot / Resources / Tasks / Networking are gone. Tengu `download` now shows up in Downloads like Demon.
